...
fasta_file <- "data/input/selected_transcripts/transcripts_1tx.fa"
tx <- readDNAStringSet(fasta_file)
dataset_name <- "dataset1"
outdir <- "data/input/sim/polyester_design/dataset1"
...
# validate transcript and genes length
if (length(transcript_id) != 900) {
  stop("Dataset 1 expects 900 transcripts.")
}
if (!all(table(gene_id) == 1)) {
  stop("Dataset 1 FASTA must contain exactly one transcript per gene.")
}
...
# Helper for expanding each block spec into transcript-level design rows.
expand_block <- function(block_df, expected_n, block_name) {
  ...
  if (nrow(out) != expected_n) {
    stop(sprintf("Internal design error for %s: expected %d rows, got %d.",
...
#   make design blocks easy x300.
fc_label <- c("1_1", "1_2", "2_1", "1_4", "4_1")
group_easy <- data.frame(
  transcript_type = "easy",
  fc_label = rep(fc_label, 3),
  coverage = rep(coverage, 5),
  n = rep(c(40L, 15L, 15L, 15L, 15L), 3)
) %>% arrange(fc_label, coverage)
block_easy <- expand_block(group_easy, 300, "block_easy")

#   make design blocks complex x600.
group_complex <- data.frame(
  transcript_type = "complex",
  fc_label = rep(fc_label, 3),
  coverage = rep(coverage, 5),
  n = rep(c(80L, 30L, 30L, 30L, 30L), 3)
) %>% arrange(fc_label, coverage)
block_complex <- expand_block(group_complex, 600, "block_complex")

# transcript design
tx_design <- rbind(block_easy, block_complex)
...
# confirm transcript design completeness and write to file
...
count_matrix <- matrix(0L,
  ...
  ncol = length(sample_ids), dimnames = list(NULL, sample_ids)
)
