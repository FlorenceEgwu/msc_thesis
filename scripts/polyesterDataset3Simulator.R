...
fasta_file <- "data/input/selected_transcripts/transcripts_3tx.fa"
tx <- readDNAStringSet(fasta_file)
dataset_name <- "dataset3"
outdir <- "data/input/sim/polyester_design/dataset3"
...
if (length(transcript_id) != 1800) {
  stop(sprintf("Dataset 3 expects 1800 transcripts."))
}
...
# First transcript block in gene (increasing expression in second group)
fc_label1 <- c("1_1", "1_2", "1_4", "2_4")
group1_easy <- data.frame(
  transcript_type = "easy",
  fc_label = rep(fc_label1, 3),
  coverage = rep(coverage, 4),
  n = rep(25, 12) # 25txs per FC group for easy
...
group1_complex <- data.frame(
  ...
  fc_label = rep(fc_label1, 3),
  coverage = rep(coverage, 4),
  n = rep(50, 12) # 50txs per FC group for complex
) %>% arrange(fc_label, coverage)
block1_complex <- expand_block(group1_complex, 600, "block1_complex")

# second transcript block in gene (increasing expression in first group)
fc_label2 <- c("1_1", "2_1", "4_1", "4_2")
group2_easy <- data.frame(
  transcript_type = "easy",
  fc_label = rep(fc_label2, 3),
  coverage = rep(coverage, 4),
  n = rep(25, 12)
) %>% arrange(fc_label, coverage)
